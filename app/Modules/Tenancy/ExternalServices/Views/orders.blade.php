                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3">
                            <div class="card-box project-box">
                                <!-- Title-->
                                <h4 class="mt-0"><a class="text-dark">{{ trans('main.order').': '.$order->id }}</a></h4>
                                <p class="text-muted text-uppercase"><i class="mdi mdi-clock-outline"></i> <small>{{ $order->created_at }}</small></p>
                                <div class="text-success mb-3"> <i class="mdi mdi-bell-outline"></i> <span class="badge pt-1 bg-soft-success"> {{ $order->status }}</span></div>
                                <!-- Desc-->
                                <p class="text-muted font-13 mb-2 sp-line-2">
                                    <i class="mdi mdi-content-paste"></i> {{ trans('main.orderItems') }}: 
                                    @foreach($order->items as $key=> $item)
                                    <div class="row">
                                        <div class="col-8">
                                            {{ $key+1 .'- '}} <a href="javascript:void(0);" class="font-weight-bold text-muted">{{ $item['name'] }}</a>
                                        </div>
                                        <div class="col-4">
                                            <a href="javascript:void(0);" class="font-weight-bold text-muted">{{ trans('main.quantity').': '. $item['quantity'] }}</a>
                                        </div>
                                    </div>
                                    @endforeach
                                </p>
