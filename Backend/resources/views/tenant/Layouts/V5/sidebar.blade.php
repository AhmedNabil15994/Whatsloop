<div class="menuCpanel active">
	...
	<ul class="linksCpanel">
		...
		<li titlehover="link1">
			<a href="{{ URL::to('/dashboard') }}"><i class="flaticon-home"></i> {{ trans('main.dashboard') }}</a>
		</li>

		@if(\Helper::checkRules('list-livechat'))
		<li titlehover="link2" id="foo">
			<a href="{{ URL::to('/livechat') }}"><i class="flaticon-statistics"></i> {{ trans('main.livechat') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('salla-customers,salla-products,salla-orders,salla-abandoned-carts,salla-reports,salla-templates'))
		<li titlehover="link3">
			<a href="#" class="subToggle"><i class="flaticon-shopping-bag"></i> {{ trans('main.salla') }} <i class="arrowLeft flaticon-left-arrow"></i></a>
			<ul class="subMenu">
				<li><a href="{{ URL::to('/profile/services?type=salla') }}">{{ trans('main.service_tethering') }}</a></li>
				@if(\Helper::checkRules('salla-customers'))
				<li><a href="{{ URL::to('/services/salla/customers') }}">{{ trans('main.customers') }}</a></li>
				@endif
				@if(\Helper::checkRules('salla-products'))
				<li><a href="{{ URL::to('/services/salla/products') }}">{{ trans('main.products') }}</a></li>
				@endif
				@if(\Helper::checkRules('salla-orders'))
				<li><a href="{{ URL::to('/services/salla/products') }}">{{ trans('main.orders') }}</a></li>
				@endif
				@if(\Helper::checkRules('salla-abandoned-carts'))
				<li><a href="{{ URL::to('/services/salla/abandonedCarts') }}">{{ trans('main.abandonedCarts') }}</a></li>
				@endif
				@if(\Helper::checkRules('salla-reports'))
				<li><a href="{{ URL::to('/services/salla/reports') }}">{{ trans('main.notReports') }}</a></li>
				@endif
				@if(\Helper::checkRules('salla-templates'))
				<li><a href="{{ URL::to('/services/salla/templates') }}">{{ trans('main.templates') }}</a></li>
				@endif
			</ul>
		</li>
		@endif
		...
		<li titlehover="link4">
			...
			<ul class="subMenu">
				<li><a href="{{ URL::to('/profile/services?type=zid') }}">{{ trans('main.service_tethering') }}</a></li>
				@if(\Helper::checkRules('zid-customers'))
				<li><a href="{{ URL::to('/services/zid/customers') }}">{{ trans('main.customers') }}</a></li>
				@endif
				@if(\Helper::checkRules('zid-products'))
				<li><a href="{{ URL::to('/services/zid/products') }}">{{ trans('main.products') }}</a></li>
				@endif
				@if(\Helper::checkRules('zid-orders'))
				<li><a href="{{ URL::to('/services/zid/products') }}">{{ trans('main.orders') }}</a></li>
				@endif
				@if(\Helper::checkRules('zid-abandoned-carts'))
				<li><a href="{{ URL::to('/services/zid/abandonedCarts') }}">{{ trans('main.abandonedCarts') }}</a></li>
				@endif
				@if(\Helper::checkRules('zid-reports'))
				<li><a href="{{ URL::to('/services/zid/reports') }}">{{ trans('main.notReports') }}</a></li>
				@endif
				@if(\Helper::checkRules('zid-templates'))
				<li><a href="{{ URL::to('/services/zid/templates') }}">{{ trans('main.templates') }}</a></li>
				@endif
			</ul>
		...
		<li titlehover="link5">
			<a  class="subToggle"><i class="flaticon-layer"></i> {{ trans('main.whatsappOrders') }} <i class="arrowLeft flaticon-left-arrow"></i></a>
			<ul class="subMenu">
				@if(\Helper::checkRules('whatsapp-products'))
				<li><a href="{{ URL::to('/whatsappOrders/products') }}">{{ trans('main.products') }} </a></li>
				@endif
				@if(\Helper::checkRules('whatsapp-orders'))
				<li><a href="{{ URL::to('/whatsappOrders/orders') }}">{{ trans('main.orders') }} </a></li>
				@endif
			</ul>
		</li>
		@endif
		...
		<li titlehover="link6">
			<a href="{{ URL::to('/bots') }}"><i class="flaticon-robot"></i> {{ trans('main.chatBot') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-templates'))
		<li titlehover="link7">
			<a href="{{ URL::to('/templates') }}"><i class="flaticon-layer"></i> {{ trans('main.templates') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-replies'))
		<li titlehover="link8">
			<a href="{{ URL::to('/replies') }}"><i class="flaticon-chat-bubble"></i> {{ trans('main.replies') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-categories'))
		<li titlehover="link9">
			<a href="{{ URL::to('/categories') }}"><i class="flaticon-menu"></i> {{ trans('main.categories') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-contacts'))
		<li titlehover="link10">
			<a href="{{ URL::to('/contacts') }}"><i class="flaticon-send"></i> {{ trans('main.contacts') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-group-numbers,add-number-to-group,list-groupNumberReports'))
		<li titlehover="link11">
			<a href="#" class="subToggle"><i class="flaticon-add-group"></i> {{ trans('main.groupNumbers') }} <i class="arrowLeft flaticon-left-arrow"></i></a>
			<ul class="subMenu">
        ...
        <li titlehover="link12">
			<a href="#" class="subToggle"><i class="flaticon-edit"></i> {{ trans('main.groupMsgs') }} <i class="arrowLeft flaticon-left-arrow"></i></a>
			<ul class="subMenu">
				@if(\Helper::checkRules('list-group-messages'))
				<li><a href="{{ URL::to('/groupMsgs') }}">{{ trans('main.groupMsgs') }}</a></li>
				@endif
				@if(\Helper::checkRules('add-group-message'))
				<li><a href="{{ URL::to('/groupMsgs/add') }}">{{ trans('main.sendNewMessage') }}</a></li>
				@endif
			</ul>
		</li>
		@endif
		...
		<li titlehover="link13">
			<a href="{{ URL::to('/statuses') }}"><i class="flaticon-mode"></i> {{ trans('main.statuses') }}</a>
		</li>
		@endif
		...
		<li titlehover="link15">
			<a href="{{ URL::to('/tickets') }}"><i class="flaticon-tag"></i> {{ trans('main.tickets') }}</a>
		</li>
		@endif
		
		@if(\Helper::checkRules('list-storage'))
		<li titlehover="link16">
			<a href="{{ URL::to('/storage') }}"><i class="flaticon-folder"></i> {{ trans('main.storage') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-invoices'))
		<li titlehover="link17">
			<a href="{{ URL::to('/invoices') }}"><i class="flaticon-invoice"></i> {{ trans('main.subs_invoices') }}</a>
		</li>
		@endif

		@if(\Helper::checkRules('list-users,list-groups'))
        <li titlehover="link18">
			<a href="#" class="subToggle"><i class="flaticon-group"></i> {{ trans('main.users') }} <i class="arrowLeft flaticon-left-arrow"></i></a>
			<ul class="subMenu">
				@if(\Helper::checkRules('list-users'))
				<li><a href="{{ URL::to('/users') }}">{{ trans('main.users') }}</a></li>
				@endif
				@if(\Helper::checkRules('list-groups'))
				<li><a href="{{ URL::to('/groups') }}">{{ trans('main.groups') }}</a></li>
				@endif
			</ul>
		</li>
		@endif
        ...
        <li titlehover="link19">
			<a href="#" class="subToggle"><i class="flaticon-settings"></i> {{ trans('main.website_setting') }} <i class="arrowLeft flaticon-left-arrow"></i></a>
			<ul class="subMenu">
				@if(\Helper::checkRules('apiSetting'))
				<li><a href="{{ URL::to('/profile/apiSetting') }}">{{ trans('main.api_setting') }}</a></li>
				@endif
				@if(\Helper::checkRules('apiGuide'))
				<li><a href="{{ URL::to('/profile/apiGuide') }}">{{ trans('main.api_guide') }}</a></li>
				@endif
				@if(\Helper::checkRules('webhookSetting'))
				<li><a href="{{ URL::to('/profile/webhookSetting') }}">{{ trans('main.webhook_setting') }}</a></li>
				@endif
			</ul>
		</li>
		@endif
		...
		<li titlehover="link20">
			<a href="{{ URL::to('/profile/personalInfo') }}"><i class="flaticon-pages"></i> {{ trans('main.account_setting') }}</a>
		</li>
		<li titlehover="link21">
			<a href="{{ URL::to('/helpCenter') }}"><i class="flaticon-life-buoy"></i> {{ trans('main.helpCenter') }}</a>
		</li>
