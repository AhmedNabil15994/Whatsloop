...
class oldwebhook extends ProcessWebhookJob{
	    		...
	    		if($message['fromMe'] == false && !str_contains($message['chatId'], '@g.us') ){	
	    			...
	    			if($message['type'] == 'buttons_response'){
		    			...
		    			if(count($botObj) == 0){
		    				...
		    				if(count($botObj) > 0){
		    					foreach ($botObj as $key => $oneBot) {
		    						if(in_array(strlen($senderMessage),[4,8])){
			    						...
			    						if(isset($emojis[0]) && !empty($emojis[0]) && in_array(strlen($oneBot->message), [4,8]) && json_encode($senderMessage) == json_encode($oneBot->message)){
		    				...
		    				foreach ($botObj as $key => $oneBots) {
		    					...
		    					if(in_array(strlen($senderMessage),[4,8])){
		    						preg_match_all('([*#0-9](?>\\xEF\\xB8\\x8F)?\\xE2\\x83\\xA3|\\xC2[\\xA9\\xAE]|\\xE2..(\\xF0\\x9F\\x8F[\\xBB-\\xBF])?(?>\\xEF\\xB8\\x8F)?|\\xE3(?>\\x80[\\xB0\\xBD]|\\x8A[\\x97\\x99])(?>\\xEF\\xB8\\x8F)?|\\xF0\\x9F(?>[\\x80-\\x86].(?>\\xEF\\xB8\\x8F)?|\\x87.\\xF0\\x9F\\x87.|..(\\xF0\\x9F\\x8F[\\xBB-\\xBF])?|(((?<zwj>\\xE2\\x80\\x8D)\\xE2\\x9D\\xA4\\xEF\\xB8\\x8F\k<zwj>\\xF0\\x9F..(\k<zwj>\\xF0\\x9F\\x91.)?|(\\xE2\\x80\\x8D\\xF0\\x9F\\x91.){2,3}))?))', $senderMessage, $emojis);
		    						if(isset($emojis[0]) && !empty($emojis[0]) && in_array(strlen($oneBots->message),[4,8]) && json_encode($senderMessage) == json_encode($oneBots->message)){
		    			...
		    			if($botPlusObjs){
		    				...
		    				if(in_array(strlen($senderMessage),[4,8])){
	    						preg_match_all('([*#0-9](?>\\xEF\\xB8\\x8F)?\\xE2\\x83\\xA3|\\xC2[\\xA9\\xAE]|\\xE2..(\\xF0\\x9F\\x8F[\\xBB-\\xBF])?(?>\\xEF\\xB8\\x8F)?|\\xE3(?>\\x80[\\xB0\\xBD]|\\x8A[\\x97\\x99])(?>\\xEF\\xB8\\x8F)?|\\xF0\\x9F(?>[\\x80-\\x86].(?>\\xEF\\xB8\\x8F)?|\\x87.\\xF0\\x9F\\x87.|..(\\xF0\\x9F\\x8F[\\xBB-\\xBF])?|(((?<zwj>\\xE2\\x80\\x8D)\\xE2\\x9D\\xA4\\xEF\\xB8\\x8F\k<zwj>\\xF0\\x9F..(\k<zwj>\\xF0\\x9F\\x91.)?|(\\xE2\\x80\\x8D\\xF0\\x9F\\x91.){2,3}))?))', $senderMessage, $emojis);
	    						if(isset($emojis[0]) && !empty($emojis[0]) && in_array(strlen($botPlusObj->message),[4,8]) && json_encode($senderMessage) == json_encode($botPlusObj->message)){
	    							$this->handleBotPlus($message,$botPlusObj,$userObj->domain,$sender);	
	    						}
	    					}else{
	    						Logger((array)$botPlusObjs);
	    						$this->handleBotPlus($message,$botPlusObj,$userObj->domain,$sender);
	    					}
		    			}
