...
class FAQControllers extends Controller {
    ...
    public function edit($id) {
        ...
        $userObj = FAQ::NotDeleted()->find($id);
        if($userObj == null) {
            return Redirect('404');
        }

        $data['data'] = FAQ::getData($userObj);
        $data['designElems'] = $this->getData();
    ...
    public function update($id) {
        ...
        $input = \Request::all();
        // dd($input);
        $dataObj = FAQ::NotDeleted()->find($id);
        if($dataObj == null) {
            return Redirect('404');
        }
    ...
    public function sort(){
        ...
        for ($i = 0; $i < count($ids) ; $i++) {
            FAQ::where('id',$ids[$i])->update(['sort'=>$sorts[$i]]);
        }
