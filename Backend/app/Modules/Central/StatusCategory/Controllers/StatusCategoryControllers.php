...
class StatusCategoryControllers extends Controller {
    ...
    public function getData(){
        $data['mainData'] = [
            ...
            'nameOne' => 'status-category',
            'modelName' => 'StatusCategory',
    ...
    public function edit($id) {
        ...
        $userObj = StatusCategory::NotDeleted()->find($id);
        if($userObj == null) {
            return Redirect('404');
        }

        $data['data'] = StatusCategory::getData($userObj);
        $data['designElems'] = $this->getData();
        $data['designElems']['mainData']['title'] = trans('main.edit') . ' '.trans('main.categories') ;
        $data['designElems']['mainData']['icon'] = 'fa fa-pencil-alt';
        return view('Central.StatusCategory.Views.edit')->with('data', (object) $data);      
    }
    ...
    public function update($id) {
        ...
        $input = \Request::all();
        $dataObj = StatusCategory::NotDeleted()->find($id);
        if($dataObj == null) {
            return Redirect('404');
        }
    ...
    public function add() {
        ...
        return view('Central.StatusCategory.Views.add')->with('data', (object) $data);
    }
    ...
    public function create() {
        ...
        $dataObj = new StatusCategory;
        $dataObj->title_ar = $input['title_ar'];
        $dataObj->title_en = $input['title_en'];
        $dataObj->description_en = $input['description_en'];
        $dataObj->description_ar = $input['description_ar'];
        $dataObj->sort = StatusCategory::newSortIndex();
        $dataObj->status = $input['status'];
    ...
    public function delete($id) {
        $id = (int) $id;
        $dataObj = StatusCategory::getOne($id);
    ...
    public function fastEdit() {
        ...
        foreach ($input['data'] as $item) {
            $col = $item[1];
            $dataObj = StatusCategory::find($item[0]);
            $dataObj->$col = $item[2];
            $dataObj->updated_at = DATE_TIME;
            $dataObj->updated_by = USER_ID;
            $dataObj->save();
    ...
    public function sort(){
        ...
        for ($i = 0; $i < count($ids) ; $i++) {
            StatusCategory::where('id',$ids[$i])->update(['sort'=>$sorts[$i]]);
        }
