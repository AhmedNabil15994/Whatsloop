            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-horizontal" method="POST" action="{{ URL::to('/'.$data->designElems['mainData']['url'].'/update/'.$data->data->id) }}">
                        ...
                        <div class="form-group row mb-3">
                            <label for="inputEmail3" class="col-3 col-form-label">{{ trans('main.assignment') }} :</label>
                            <div class="col-9">
                                <select class="form-control" data-toggle="select2" name="assignment[]" multiple {{ !IS_ADMIN ? 'disabled' : '' }}>
                                    <option value="">{{ trans('main.choose') }}</option>
                                    @foreach($data->assigns as $assign)
                                    <option value="{{ $assign->id }}" {{ in_array($assign->id, $data->data->assignment) ? 'selected' : '' }}>{{ '#'.$assign->id .' - '. $assign->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-9">
                                <select class="form-control" data-toggle="select2" name="priority_id">
                                    <option value="">{{ trans('main.choose') }}</option>
                                    <option value="1" {{ 1 == $data->data->priority_id ? 'selected' : '' }}>{{ trans('main.low') }}</option>
                                    <option value="2" {{ 2 == $data->data->priority_id ? 'selected' : '' }}>{{ trans('main.medium') }}</option>
                                    <option value="3" {{ 3 == $data->data->priority_id ? 'selected' : '' }}>{{ trans('main.high') }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row mb-3">
...
<script src="{{ asset('tenancy/assets/components/comments.js') }}"></script>      
@endsection
