...
class StatusControllers extends Controller {
    ...
    public function getData(){
        ...
        $categories = StatusCategory::dataList(1)['data'];
        $data['searchData'] = [
            'id' => [
                'type' => 'text',
    ...
    protected function validateInsertObject($input){
        ...
        $message = [
            ...
            'status.required' => trans('main.statusValidate'),
        ];
    ...
    public function edit($id) {
        ...
        $userObj = Status::NotDeleted()->find($id);
        if($userObj == null) {
            return Redirect('404');
        }

        $data['data'] = Status::getData($userObj);
        $data['designElems'] = $this->getData();
        $data['designElems']['mainData']['title'] = trans('main.edit') . ' '.trans('main.changeLogs') ;
        $data['designElems']['mainData']['icon'] = 'fa fa-pencil-alt';
        $data['categories'] = StatusCategory::dataList(1)['data'];
        return view('Central.Status.Views.edit')->with('data', (object) $data);      
    }
    ...
    public function update($id) {
        ...
        $input = \Request::all();
        // dd($input);
        $dataObj = Status::NotDeleted()->find($id);
        if($dataObj == null) {
            return Redirect('404');
        }
    ...
    public function add() {
        $data['designElems'] = $this->getData();
        $data['designElems']['mainData']['title'] = trans('main.add') . ' '.trans('main.statuses') ;
        $data['designElems']['mainData']['icon'] = 'fa fa-plus';
        $data['categories'] = StatusCategory::dataList(1)['data'];
        return view('Central.Status.Views.add')->with('data', (object) $data);
    }
    ...
    public function create() {
        ...
        $dataObj->description_en = $input['description_en'];
        $dataObj->category_id = $input['category_id'];
        $dataObj->sort = Status::newSortIndex();
    ...
    public function sort(){
        ...
        for ($i = 0; $i < count($ids) ; $i++) {
            Status::where('id',$ids[$i])->update(['sort'=>$sorts[$i]]);
        }
