        ...
        <div class="col-md-12">
            <div class="form">
                <div class="card-body">
                    ...
                    <form class="formPayment" method="POST" action="{{ URL::to('/categories/update/'.$data->data->id) }}">
                        @csrf
                            ...
                            <div class="col-md-9">
                                <div class="selectStyle colors">
                                    <select data-toggle="select2" data-style="btn-outline-myPR" name="color_id">
                                        ...
                                        <option value="{{ $color['id'] }}" {{ $data->data->color_id == $color['id'] ? 'selected' : '' }} style="background: {{ $color['color'] }}">{{ $color['title'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <input type="text" value="{{ $data->data->name_ar }}" name="name_ar" placeholder="{{ trans('main.titleAr') }}">
                                <span class="m-form__help LastUpdate float-right mt-1 mb-0">{{ trans('main.created_at') }} :  {{ $data->data->created_at }}</span>
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <input type="text" value="{{ $data->data->name_en }}" name="name_en" placeholder="{{ trans('main.titleEn') }}">
                                <span class="m-form__help LastUpdate float-right mt-1 mb-0">{{ trans('main.created_at') }} :  {{ $data->data->created_at }}</span>
                            </div>
                        </div>
                        <hr class="mt-5">
