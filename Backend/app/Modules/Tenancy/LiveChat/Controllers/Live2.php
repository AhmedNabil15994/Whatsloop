...
class Live2 extends Controller {
    ...
    public function sendMessage(Request $request) {
        ...
        $senderStatus = trans('main.channel'). ' #';
        if($userObj->domain != 'T1002'){
            ...
            $tenantId = '01c5e3a0-24ad-420e-b3d7-b4e09e0301e7';
        }
        ...
        if($input['type'] == 1){
            ...
            if ($request->hasFile('file')) {
                ...
                $bodyData = config('app.BASE_URL').'/public/uploads/'.$tenantId.'/chats/'.$fileName;
                if($userObj->domain != 'T1002'){
                    $bodyData = str_replace('wloop.net','23d2-156-205-240-240.ngrok.io',$bodyData);
                }
            ...
            if ($request->hasFile('file')) {
                ...
                if(isset($input['size']) && !empty($input['size'])){
                    ...
                    $destinationPath = public_path().'/uploads/'.$tenantId.'/chats/' . $fileName.'.ogg';
                    ...
                    $bodyData = config('app.BASE_URL').'/public/uploads/'.$tenantId.'/chats/' . $fileName.'.ogg';
                    if($userObj->domain != 'T1002'){
                        $bodyData = str_replace('wloop.net','23d2-156-205-240-240.ngrok.io',$bodyData);
                    }
                    $message_type = "sound";
