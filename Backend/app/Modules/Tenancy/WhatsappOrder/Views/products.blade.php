        ...
        <div class="row row-sm">
            ...
            <div class="col-12 col-sm-6 col-md-6 col-lg-6 col-xl-4">
                <div class="product-card card overflow-hidden">
                    <img class="w-100 mt-0" style="height: 200px;" src="{{ $product->images != null ? $product->images[0] : asset('images/not-available.jpg') }}" alt="product-image"/>
                    <div class="card-body h-100">
                        <h3 class="h6 mb-2 font-weight-bold text-uppercase">{{ $product->name }}</h3>
                        <div class="d-flex">
                            <h4 class="h5 w-50 font-weight-bold text-danger">{{ $product->price . '  ' . $product->currency }}</h4>
                        </div>
                        <button class="btn btn-primary btn-block mb-0 mt-4">
                            <i class="fe fe-shopping-cart mr-1"></i>
                            {{ trans('main.quantity') }} : {{ $product->quantity }} {{ trans('main.pieces') }}
                        </button>
                    </div>
                </div>
            </div>
