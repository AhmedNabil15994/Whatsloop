...
class DelayedGroupMessages extends Command
{
    ...
    public function handle()
    {
        ...
        $messages = GroupMsg::NotDeleted()->where('later',1)->whereRaw('contacts_count != (sent_count+unsent_count)')->get();
        
        foreach ($messages as $message) {
            $dataObj = (array) GroupMsg::getData($message);
            $chunks = 400;
            if($dataObj['publish_at'] <= $now){
                $contacts = Contact::NotDeleted()->where('group_id',$message->group_id)->where('status',1)->chunk($chunks,function($data) use ($dataObj){
